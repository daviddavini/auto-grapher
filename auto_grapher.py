...
def apply_forces(node_list, graph, list_graph, partition, do_intersecting_force, dt):
...
electron_const = 30 #30,  25
intersecting_const = 25
electron_force_power = 1 #1
max_elec_force_dist = 100 #100
drag_const = 0.135 #0.1   0.08
edge_force = 20 #20
spring_length = 30 #30
dt = 0.3 #0.35,  0.5
mass = 1
...
def run_graph(function, mod, name="graph", show=True, filename=-1, wait=False):
    if not filename == -1:
        if do_numbers:
    ...
    while not stop:
        ...
        apply_forces(node_list, graph, list_graph, partition, do_intersecting_force, dt)
